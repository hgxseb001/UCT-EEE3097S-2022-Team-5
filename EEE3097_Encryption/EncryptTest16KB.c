...
{
    int encryptMatchCounter = 0;
    int failedDecryptionCounter = 0;
    ...
    for (int k = 0; k < 4; k++) //1024; k++)
    {
        createPkeys(testValues[k]);

        for (int i = 0; i < 1024; i++)
        {
            encrypted = encryptBlock(testValues[i]);
            if (testValues[i] == encrypted)
            {
                encryptMatchCounter++;
                printf("encrypted match at pos %d using %x\n",i , testValues[i]);
            }

            //decrypted = decryptBlock(encrypted);
            //if (testValues[i] != decrypted)
            //{
                //failedDecryptionCounter++;
                //printf("decryption failed at pos %d using %x\n",i , testValues[i]);
            //}
        }
    }
    clock_t end = clock();
    ...
    printf("Test completed with %d Failed Encryptions and %d Failed Decryptions", encryptMatchCounter, failedDecryptionCounter);
    return 0;
